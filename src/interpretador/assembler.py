...
def imm_to_bin(value: int, num_bits: int) -> str:
    # Para immediatos de memória / aritméticos usamos complemento de dois (signed)
    val = int(value)
    if val < 0:
        val = (1 << num_bits) + val
    return format(val & ((1 << num_bits) - 1), f'0{num_bits}b')
...
def assemble_line(assembly_line: str) -> str:
    ...
    if t == 'R':
        ...
        if mnemonic == 'JR':
            if len(parts) != 2:
                raise ValueError("Formato inválido para JR. Ex: JR Rn")
            Rd_bin = '0000'
            Rs_bin = reg_to_bin(parts[1])
            Rt_bin_4 = '0000'
        else:
            ...
            Rd_bin = reg_to_bin(parts[1])
            Rs_bin = reg_to_bin(parts[2])
            Rt_bin_4 = reg_to_bin(parts[3])
        F3 = Rt_bin_4 + '0' * 12
        return op_bin + Rd_bin + Rs_bin + F3

    # I_ARITH or I_MEM or I_BRANCH or I_CONST
    if t in ['I_ARITH', 'I_MEM', 'I_BRANCH', 'I_CONST']:
        if op_info['type'] == 'I_ARITH':
            # Format: MN Rt, Rs, imm  (imm signed 16)
            if len(parts) != 4:
                raise ValueError(f"Formato inválido para {mnemonic}. Ex: {mnemonic} Rdest, Rsrc, imm")
            Rt_bin = reg_to_bin(parts[1])
            Rs_bin = reg_to_bin(parts[2])
        ...
        if op_info['type'] == 'I_MEM':
            # LW Rt, offset(Rs) -> parts parsed as [LW, Rt, offset, Rs]
            if len(parts) != 4:
                raise ValueError(f"Formato inválido para {mnemonic}. Ex: {mnemonic} Rt, offset(Rs)")
            Rt_bin = reg_to_bin(parts[1])
            offset = int(parts[2])
        ...
        if op_info['type'] == 'I_BRANCH':
            # JEQ/JNE: MN Rs, Rt, target (target is absolute byte address)
            if mnemonic in ['JEQ', 'JNE']:
                if len(parts) != 4:
                    raise ValueError(f"Formato inválido para {mnemonic}. Ex: {mnemonic} Rs, Rt, target")
                Rt_bin = reg_to_bin(parts[2])
                Rs_bin = reg_to_bin(parts[1])
                # Target is treated as UNSIGNED 16-bit absolute byte address
                target = int(parts[3])
                if target < 0 or target > 0xFFFF:
                    raise ValueError("Target fora do intervalo 0..65535 para instrução de branch.")
                Imm_bin = format(target, '016b')
                return op_bin + Rt_bin + Rs_bin + Imm_bin
            elif mnemonic in ['J', 'JAL']:
                if len(parts) != 2:
                    raise ValueError(f"Formato inválido para {mnemonic}. Ex: {mnemonic} target")
                target = int(parts[1])
                if target < 0 or target > 0xFFFF:
                    raise ValueError("Target fora do intervalo 0..65535 para J/JAL.")
                Rt_bin = '0000'
                Rs_bin = '0000'
                Imm_bin = format(target, '016b')
                return op_bin + Rt_bin + Rs_bin + Imm_bin
        ...
        if op_info['type'] == 'I_CONST':
            # Format: MN Rt, imm  (unsigned 16)
            if len(parts) != 3:
                raise ValueError(f"Formato inválido para {mnemonic}. Ex: {mnemonic} Rt, imm")
            Rt_bin = reg_to_bin(parts[1])
            imm_val = int(parts[2]) & 0xFFFF
            imm_16 = format(imm_val, '016b')
            imm_h = imm_16[:8]
            imm_l = imm_16[8:]
            imm_l_12 = imm_l + '0' * 4
            return op_bin + Rt_bin + imm_h + imm_l_12
